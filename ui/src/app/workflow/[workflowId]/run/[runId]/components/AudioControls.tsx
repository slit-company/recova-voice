...
export const AudioControls = ({
  ...
  const hasValidDevices =
    ...
    audioInputs.some((device) => device.label && device.label.trim() !== "");
  ...
  useEffect(() => {
    if (hasValidDevices && !selectedAudioInput) {
      const firstValidDevice = audioInputs.find(
  ...
  if (!hasValidDevices) {
        ...
        <div className="flex flex-col items-center justify-center space-y-4 p-8">
          ...
          <div className="text-center space-y-2">
            <p className="text-foreground font-medium">
              {t("runDetail.micDenied")}
            </p>
            <p className="text-sm text-muted-foreground max-w-md">
              {t("runDetail.micDeniedHelp")}
            </p>
          </div>
          <Button
          ...
          >
            {isRequestingPermission ? (
              <>
                ...
                {t("runDetail.waitingPermission")}
              </>
            ) : (
              <>
                <Mic className="h-5 w-5 mr-2" />
                {t("runDetail.tryAgain")}
              </>
            )}
          </Button>
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center space-y-4 p-8">
        <div className="text-center space-y-2">
          <p className="text-foreground font-medium">
            {t("runDetail.audioRequired")}
          </p>
          <p className="text-sm text-muted-foreground">
            {isRequestingPermission
              ? t("runDetail.allowMic")
              : t("runDetail.grantMic")}
          </p>
        </div>
        <Button
      ...
      {!connectionActive ? (
        <>
          <button
            ...
            aria-label={t("runDetail.startCall")}
          >
          ...
          <p className="text-sm font-medium text-foreground">
            {t("runDetail.startCall")}
          </p>
        </>
      ) : (
        <>
          <p className="text-sm text-muted-foreground">
            {t("runDetail.callInProgress")}
          </p>
          <button
            onClick={stop}
            ...
            aria-label={t("runDetail.endCall")}
          >
            <div className="relative flex items-center justify-center h-full">
