...
import {
  MediaPreviewButton,
  MediaPreviewDialog,
} from "@/components/MediaPreviewDialog";
...
import {
  ...
  RealtimeFeedback,
  WorkflowRunLogs,
} from "@/components/workflow/conversation";
...
const RUN_SHELL_HEIGHT_CLASS =
  "h-[calc(100svh-49px)] min-h-[calc(100svh-49px)] max-h-[calc(100svh-49px)]";
...
function getTranscriptMetrics(
  ...
  const userTurns = events.filter(
    (event) => event.type === "rtf-user-transcription" && event.payload.final,
  ).length;
  const botTurns = events.filter(
    (event) => event.type === "rtf-bot-text",
  ).length;
  const toolCalls = events.filter(
    (event) => event.type === "rtf-function-call-end",
  ).length;
  ...
  return (
    <Card className="border-border">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg">{t("runDetail.metrics")}</CardTitle>
      </CardHeader>
      <CardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <MetricCard
          label={t("runDetail.duration")}
          value={formatDuration(costInfo?.call_duration_seconds)}
        />
        <MetricCard
          label={t("runDetail.tokenUsage")}
          value={
            costInfo?.dograh_token_usage != null
              ? costInfo.dograh_token_usage.toLocaleString()
              : "N/A"
          }
        />
        <MetricCard
          label={t("runDetail.userTurns")}
          value={String(metrics.userTurns)}
        />
        <MetricCard
          label={t("runDetail.botTurns")}
          value={String(metrics.botTurns)}
        />
        <MetricCard
          label={t("runDetail.toolCalls")}
          value={String(metrics.toolCalls)}
        />
        <MetricCard
          label={t("runDetail.nodesVisited")}
          value={String(metrics.visitedNodes)}
        />
      </CardContent>
    </Card>
  );
...
export default function WorkflowRunPage() {
  ...
  useEffect(() => {
    const fetchWorkflowRun = async () => {
      ...
      const runData = {
        ...
        initial_context:
          (response.data?.initial_context as Record<string, string> | null) ??
          null,
        gathered_context:
          (response.data?.gathered_context as Record<string, string> | null) ??
          null,
        logs: (response.data?.logs as WorkflowRunLogs | null) ?? null,
        annotations:
          (response.data?.annotations as Record<string, unknown> | null) ??
          null,
      };
  ...
  const showRunDetailsView = Boolean(
  ...
  if (isLoading) {
    ...
    returnValue = (
      <div
        className={`flex ${RUN_SHELL_HEIGHT_CLASS} min-h-0 w-full overflow-hidden bg-background`}
      >
        <div className="min-w-0 flex-1 overflow-y-auto">
          <div className="mx-auto w-full max-w-4xl space-y-6 p-6">
            <Card className="border-border">
              ...
              <CardContent>
                <p className="text-muted-foreground mb-8">
                  {isTextChatRun
                    ? t("runDetail.textChatDescription")
                    : t("runDetail.completedDescription")}
                </p>
                ...
                <div className="flex flex-wrap gap-4">
                  {!isTextChatRun && (
                    <>
                      ...
                      <div className="flex items-center gap-2 border-l border-border pl-4">
                        <span className="text-sm text-muted-foreground">
                          {t("runDetail.download")}
                        </span>
                        <Button
                          onClick={() =>
                            downloadFile(workflowRun?.transcript_url ?? null)
                          }
                          disabled={
                            !workflowRun?.transcript_url ||
                        ...
                        <Button
                          onClick={() =>
                            downloadFile(workflowRun?.recording_url ?? null)
                          }
                          disabled={
                            !workflowRun?.recording_url || !auth.isAuthenticated
                          }
                          size="sm"
                          className="gap-2"
                        >
                          <Video className="h-4 w-4" />
                          {t("runDetail.recording")}
                        </Button>
                      </div>
                    </>
                  )}
                  {workflowRun?.gathered_context?.trace_url && (
                    <div
                      className={`flex items-center gap-2 ${isTextChatRun ? "" : "border-l border-border pl-4"}`}
                    >
                      <span className="text-sm text-muted-foreground">
                        {t("runDetail.trace")}
                      </span>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <ContextDisplay
                title={t("runDetail.initialContext")}
                context={workflowRun?.initial_context ?? null}
              />
              <ContextDisplay
                title={t("runDetail.gatheredContext")}
                context={workflowRun?.gathered_context ?? null}
              />
            </div>

            {workflowRun?.annotations &&
              Object.keys(workflowRun.annotations).length > 0 && (
                <ContextDisplay
                  title={t("runDetail.qaResults")}
                  context={
                    workflowRun.annotations as Record<
                      string,
      ...
      <div className="flex h-full items-center justify-center p-6">
        <Card className="w-full max-w-xl border-border">
          <CardHeader className="space-y-2">
            <CardTitle className="text-2xl">
              {t("runDetail.unavailable")}
            </CardTitle>
            <p className="text-sm text-muted-foreground">
              {t("runDetail.unavailableDescription")}
            </p>
          </CardHeader>
          <CardFooter>
  ...
  return (
    <WorkflowLayout>
      ...
      {showRunDetailsView && (
        <OnboardingTooltip
          title={t("runDetail.customizeTitle")}
          targetRef={customizeButtonRef}
          message={t("runDetail.customizeMessage")}
